...
@section('js')
    <script>
                        ...
                        },
                        {
                            ...
                            formatter: (_, row) => {
                                ...
                                if (value && value.startsWith('upload/others/')) {
                                    return gridjs.html(
                                        `<img src="/storage/${value}" alt="Ảnh" style="max-width: 100px; height: auto;" />`
                                        );
                                } else {
                                    const truncatedValue = value.length > 100 ? value.substring(0,
                                        100) + '...' : value;
        ...
        <a href="${editRoute.replace(':id', row.cells[0].data)}" class="edit-link" title="Sửa">
        ...
        <form id="delete-form-${row.cells[0].data}" action="/admin/others/${row.cells[0].data}" method="POST" style="display: inline-block;">
            ...
            <button type="submit" onclick="return confirm('bạn có muốn xóa?')" class="btn btn-danger" title="Xóa">
